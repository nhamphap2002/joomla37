 ...
 * @since  1.6
 */
class BookingModelOrders extends JModelList
{
/**
	* Constructor.
	*
	...
	*/
	public function __construct($config = array())
	{
		if (empty($config['filter_fields']))
		{
			$config['filter_fields'] = array(
				...
				'number_children', 'a.`number_children`',
				'order_note', 'a.`order_note`',
				'name_customer', 'a.`name_customer`',
				'phome', 'a.`phome`',
				'email', 'a.`email`',
				'idroom', 'a.`idroom`',
				'total', 'a.`total`',
				'parentroom', 'a.`parentroom`',
			);
		}
	...
	protected function getListQuery()
	{
		...
		$query->from('`#__booking_order` AS a');
	...
	public function getItems()
	{
		...
		foreach ($items as $oneItem) {
			...
			if (isset($oneItem->idroom)) {
				$values = explode(',', $oneItem->idroom);
			...
			$oneItem->idroom = !empty($textValue) ? implode(', ', $textValue) : $oneItem->idroom;

			}

			if (isset($oneItem->parentroom)) {
				$values = explode(',', $oneItem->parentroom);
			...
			$oneItem->parentroom = !empty($textValue) ? implode(', ', $textValue) : $oneItem->parentroom;
